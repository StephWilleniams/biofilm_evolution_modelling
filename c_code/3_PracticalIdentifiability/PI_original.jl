...
[println(x_opt[i]/theta_true[i]) for i in 1:14]

do_univariate = true
if do_univariate
    modifier = LinRange(1-uniRange,1+uniRange,N_varies)
    for ii in 3:14 # Loop on parameters
        out = zeros(N_varies)
        for jj in eachindex(out) # Loop on values of fixed param
            ...
            function llhood(params_vary1,_)
                ...
                params = [params_vary1[1:ind-1]..., params_const[1]..., params_vary1[ind:end]...]
        ...
        savefig("c_code/figures/PI/PI_basicTest_$ii.png")
    end
end
