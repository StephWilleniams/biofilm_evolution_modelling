...
using GlobalSensitivity, QuasiMonteCarlo, DifferentialEquations, Plots
...
delta = 0.01
samples = 80000
lb = [(1-delta)*theta[i] for i = eachindex(theta)]
ub = [(1+delta)*theta[i] for i = eachindex(theta)]
sampler = GlobalSensitivity.SobolSample()

A,B = QuasiMonteCarlo.generate_design_matrices(samples,lb,ub,sampler)

Y = [model(A[:,j]) for j in 1:samples]

res2 = gsa(A, Y', RegressionGSA(true))

println("Outputs of Regression sensitivity")
println("########## - Non-Ranked")

println("Pearson")
println(res2.pearson)
println("Standard")
println(res2.standard_regression)
println("partial correlation")
println(res2.partial_correlation)

println("########## - Ranked")
println("Spearman")
println(res2.pearson_rank)
println("Standard rank")
println(res2.standard_rank_regression)
println("partial correlation rank")
println(res2.partial_rank_correlation)
