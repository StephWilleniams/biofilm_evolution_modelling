...
for A0 in LinRange(0.0,1.0,NpredRuns)
    ...
    #colors = palette(cgrad([:green, :blue], 1+Int(200/10))); 
    #colind=1  # 3 colors
    bindRates = LinRange(0.000001,0.1,N_bins)
    #plt = plot()
    #plot!(plt,bindRates,F0,lw=3,legend=false, color=colors[colind])
    #colind+=1
    ...
    for runs in 1:RunNumber
        ...
        #global F0 = NofT[2:1+N_bins,end]./sum(NofT[2:1+N_bins,end])
        global u0 = vcat(C0,F0,B0,S0,A0) # IVP

        EofRun[runs] = sum(F0.*bindRates)

        #println(sum(F0.*bindRates))

        # if runs % 10 == 0
        #     plot!(plt,bindRates,NofT[2+N_bins:2+2*N_bins-1,end]./sum(NofT[2+N_bins:2+2*N_bins-1,end]),lw=3,legend=false, color=colors[colind])
        #     #plot!(plt,bindRates,NofT[2:1+N_bins,end]./sum(NofT[2:1+N_bins,end]),legend=false, color=colors[colind])
        #     global colind += 1
        # end
